    ...
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white border-b border-neutral-200 px-6 py-6">
        <h2 className="text-2xl font-bold flex items-center space-x-3">
          <span className={`w-3 h-3 rounded-full ${isExecuting ? 'bg-green-500 animate-pulse' : 'bg-neutral-500'}`}></span>
          <span>Live Execution</span>
        </h2>
        <p className="text-sm text-neutral-400 mt-2">Real-time browser preview and execution monitoring</p>
      </div>

      <div className="flex-1 overflow-y-auto p-6 bg-neutral-50">
        <div className="max-w-7xl mx-auto space-y-6">
          {/* Browser Mock */}
          <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-neutral-300">
            {/* Browser Chrome */}
            <div className="bg-neutral-100 px-4 py-3 flex items-center space-x-3 border-b border-neutral-300">
              <div className="flex space-x-2">
                <div className="w-3 h-3 rounded-full bg-red-400"></div>
                <div className="w-3 h-3 rounded-full bg-yellow-400"></div>
                <div className="w-3 h-3 rounded-full bg-green-400"></div>
              </div>
              <div className="flex-1 bg-white rounded px-4 py-2 text-sm text-neutral-600 truncate border border-neutral-300">
                🔒 {currentUrl}
              </div>
            ...
            <div className="bg-white p-8 min-h-[500px] flex items-center justify-center">
              {isExecuting ? (
                <div className="text-center">
                  ...
                  <p className="text-neutral-600 font-medium">Test execution in progress...</p>
                  <p className="text-neutral-400 text-sm mt-2">Browser preview will appear here</p>
                </div>
              ) : (
            ...
            <div className="p-6">
                  ...
                  {screenshots.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {screenshots.map((screenshot) => (
                        <div key={screenshot.id} className="bg-neutral-50 rounded-xl p-4 border border-neutral-200">
                          <div className="aspect-video bg-neutral-200 rounded mb-3 flex items-center justify-center">
                            <span className="text-neutral-400 text-sm">Screenshot Preview</span>
                          </div>
                          <p className="text-sm font-medium text-neutral-700">{screenshot.label}</p>
              ...
              {activeTab === 'diff' && (
                <div>
                  ...
                  <div className="bg-neutral-50 rounded-xl p-8 border border-neutral-200 text-center text-neutral-400">
                    <svg className="w-16 h-16 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {activeTab === 'logs' && (
                <div>
                  ...
                  <div className="bg-neutral-900 rounded-xl p-4 font-mono text-sm max-h-96 overflow-y-auto">
                    {logs.length > 0 ? (
                      logs.map((log, index) => (
